...
// ── WebGL Shader Canvas (same as LoginForm) ────────────────────────────────
...
function RField({
  ...
  showToggle, visible, onToggle,
}: {
  label: string; type?: string; placeholder?: string;
  value: string; onChange: (v: string) => void;
  showToggle?: boolean; visible?: boolean; onToggle?: () => void;
}) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div
        className="rf-inp-wrap relative flex items-center rounded-xl border"
      ...
      >
        <input
          ...
          className="w-full px-4 py-2.5 bg-transparent text-sm outline-none"
          style={{ color: "#4C3A51", paddingRight: showToggle ? "2.5rem" : undefined }}
        />
        {showToggle && (
          <button
            type="button"
...
export default function RegisterForm() {
  ...
  const onSubmit = async () => {
    ...
    if (!name || !email || !password || !confirm) {
      setErrorText("All fields are required"); return;
    }
    if (password !== confirm) { setErrorText("Passwords do not match"); return; }
    setLoading(true);
    try {
      const { error } = await authClient.signUp.email({ email, password, name });
      if (error) { setErrorText(error.message || "Failed to create account"); return; }
  ...
  return (
    <>
      <style>{`
        ...
        .rf-fade { opacity:0; transform:translateY(18px); animation:rfUp 0.55s cubic-bezier(0.16,1,0.3,1) forwards; }
        @keyframes rfUp { to { opacity:1; transform:translateY(0); } }
        .rf-s1{animation-delay:.07s} .rf-s2{animation-delay:.14s}
        .rf-s3{animation-delay:.21s} .rf-s4{animation-delay:.28s}
        .rf-s5{animation-delay:.35s} .rf-s6{animation-delay:.42s}
        .rf-inp-wrap { transition: box-shadow 0.22s, border-color 0.22s; }
        .rf-inp-wrap:focus-within {
      ...
      >
        ...
        <div className="w-full md:w-[55%] h-[260px] md:h-full relative overflow-hidden flex-shrink-0 order-1 md:order-2">
          ...
          <div className="absolute inset-0 hidden md:flex items-center justify-center p-10">
            <div
              className="rf-glass rounded-2xl p-10 max-w-md text-center shadow-2xl border transition-transform duration-700 hover:scale-[1.02]"
              style={{
            ...
            >
              ...
              <p className="text-base leading-relaxed" style={{ color: "rgba(255,255,255,0.85)" }}>
                Join thousands of event organizers who trust Festo to create unforgettable experiences.
              </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          {/* Subtle grid — contained, not fixed */}
          <div
          ...
          <div className="relative z-10 w-full max-w-[420px] mx-auto px-6 md:px-0 pt-14 pb-10 md:py-8">
            {/* Logo */}
            <div className="flex items-center gap-2.5 mb-8 rf-fade rf-s1">
              <div
            ...
            {/* Heading */}
            <div className="mb-6 rf-fade rf-s2">
              <h1
            ...
            <form onSubmit={(e) => { e.preventDefault(); onSubmit(); }} className="flex flex-col gap-4">
              <div className="rf-fade rf-s3">
                <RField label="Full Name" placeholder="Jane Doe" value={name} onChange={setName} />
              </div>

              <div className="rf-fade rf-s3">
                <RField label="Email address" type="email" placeholder="you@example.com" value={email} onChange={setEmail} />
              </div>

              <div className="rf-fade rf-s4">
                <RField
                  label="Password"
                  showToggle visible={showPw} onToggle={() => setShowPw(p => !p)}
              ...
              <div className="rf-fade rf-s4">
                <RField
                  ...
                  showToggle visible={showCp} onToggle={() => setShowCp(p => !p)}
              ...
              {/* Terms */}
              <p className="text-xs leading-relaxed rf-fade rf-s4" style={{ color: "#9a8098" }}>
                By creating an account you agree to our{" "}
                <button type="button" className="font-medium transition-colors" style={{ color: "#B25068" }}
              ...
              {/* Submit */}
              <div className="rf-fade rf-s5 flex flex-col gap-3 pt-1">
                <button
                  type="submit"
                  disabled={loading}
                ...
                <SocialPill icon={<GoogleIcon />}          label="Continue with Google" />
                <SocialPill icon={<GithubIcon size={15} />} label="Continue with GitHub" />
              </div>
            </form>

            {/* Login link */}
            <p className="text-center text-sm mt-6 rf-fade rf-s6" style={{ color: "#9a8098" }}>
          ...
          {/* Footer */}
          <footer
            className="relative z-10 w-full flex justify-center md:justify-start py-4 px-6 md:px-10 mt-4"
